...
function TypingDots({ isRo }: { isRo: boolean }) {
  return (
    <span className="inline-flex items-center gap-1" aria-label={isRo ? "Scrie" : "Typing"}>
      {[0, 1, 2].map((i) => (
        <motion.span
          key={i}
...
export function HeroSection() {
  ...
  const chatMessages = isRo
    ? [
        {
          role: "user" as const,
          text: "De ce atrag mereu parteneri indisponibili emoțional?",
        },
        {
          role: "ai" as const,
          text: "Aspectele tale relaționale arată o combinație între dorința de apropiere și nevoia de protecție. Tind să apară conexiuni unde intimitatea avansează lent, pentru că vulnerabilitatea este gestionată cu mult control.",
          tags: [
            { label: "Lună în Capricorn", icon: Moon, color: "#6D4BFF" },
            { label: "Venus în aspect tensionat cu Saturn", icon: Heart, color: "#D66BFF" },
          ],
        },
      ]
    : baseChatMessages
  const floatingCards = isRo
    ? [
        {
          label: "Semn solar",
          value: "Cancer",
        ...
        {
          label: "Energie zilnică",
          value: "Claritate emoțională",
          icon: Zap,
          position: "right-[-7%] top-[45%]",
          glow: "shadow-[0_0_25px_rgba(52,211,153,0.15)]",
          iconBg: "bg-emerald-500/15",
        ...
        <div className="flex flex-col items-center text-center">
          ...
          >
            ...
            <span className="text-xs font-medium tracking-wide text-[#B69CFF]">
              {isRo ? "Ghidaj astrologic alimentat de AI" : "AI-Powered Astrology Guidance"}
            </span>
          </motion.div>
          ...
          >
            {isRo ? "Agentul tău personal de" : "Your Personal"}{" "}
            <span className="text-gradient-cosmic">
              {isRo ? "Astrologie AI" : "Astrology AI"}
            </span>{" "}
            Agent
          </motion.h1>
          ...
          >
            {isRo
              ? "Pune întrebări despre iubire, scop, compatibilitate și harta natală și primește răspunsuri profund personalizate, bazate pe date astrologice reale și AI."
              : "Ask questions about love, purpose, compatibility, and your birth chart — and get deeply personalized answers powered by real astrology data and AI."}
          </motion.p>
        ...
        >
          ...
          <div className="relative rounded-3xl border border-border bg-[#0D0820]/60 p-1 shadow-2xl shadow-[#6D4BFF]/[0.08] backdrop-blur-xl">
            ...
            <div className="rounded-[20px] bg-[#0A0618] p-5 sm:p-7">
              ...
              <div className="mb-5 flex items-center gap-3.5 rounded-2xl border border-border bg-[rgba(255,255,255,0.03)] px-4 py-3.5">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-foreground">
                    {isRo ? "Agent Love" : "Love Agent"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {isRo ? "Tipare relaționale și insight emoțional" : "Relationship patterns & emotional insight"}
                  </p>
                </div>
                <div className="ml-auto flex gap-1">
                  {(isRo ? ["Carieră", "Scop"] : ["Career", "Purpose"]).map((a) => (
                    <span
                      key={a}
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {[
                  isRo ? "Care este limbajul meu al iubirii?" : "What's my love language?",
                  isRo ? "Direcție de carieră anul acesta" : "Career path this year",
                  isRo ? "Sunt într-un Saturn return?" : "Am I in a Saturn return?",
                ].map((q) => (
                  <span
                    key={q}
              ...
              <div className="mt-4 flex items-center gap-3 rounded-2xl border border-border bg-[rgba(255,255,255,0.03)] px-4 py-3">
                <p className="flex-1 text-sm text-muted-foreground/50">
                  {isRo ? "Întreabă despre harta ta, iubire sau scop..." : "Ask about your chart, love, or purpose..."}
                </p>
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              {isRo ? "Date de hartă în timp real" : "Real-time chart data"}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="inline-block h-1 w-1 rounded-full bg-[#6D4BFF]/60" />
              {isRo ? "Privat și criptat" : "Private & encrypted"}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="inline-block h-1 w-1 rounded-full bg-[#D66BFF]/60" />
              {isRo ? "10k+ utilizatori activi" : "10k+ active users"}
            </span>
          </motion.div>
        </motion.div>
      </div>
