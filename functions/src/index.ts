...
export const oblioRetryCron = onSchedule(
  {
  ...
  async () => {
    ...
    if (!response.ok) {
      ...
      logger.error("oblio_retry_cron_failed", {
    ...
    logger.info("oblio_retry_cron_completed", {
      processed: payload && typeof payload === "object" ? (payload as { processed?: unknown }).processed : undefined,
      skippedDueToLock:
        payload && typeof payload === "object"
          ? (payload as { skippedDueToLock?: unknown }).skippedDueToLock
          : undefined,
    })
  }
)
